...
if __name__ == "__main__":
    ...
    # Load the data
    dir_data = "Data"
    data_path = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", dir_data, "iris.csv"))
    ...
    colors_flowers = {names_classes[0]: "darkblue", names_classes[1]: "turquoise", names_classes[2]: "tomato"}
    ...
    for label in names_classes:
        ...
        ax_3D.scatter(
            iris_df.loc[indices_label, "sepal_length"],
        ...
        ax_3D.text(coordinates_label[0], coordinates_label[1], coordinates_label[2], label)
    ...
    # ====================== YOUR CODE HERE ======================
    iris_data_normalized = (iris_data - np.mean(iris_data, axis=0)) / np.std(iris_data, axis=0)
    # ============================================================

    # 2. Calculation of the covariance matrix
    # ====================== YOUR CODE HERE ======================
    covariance_matrix = np.cov(iris_data_normalized, rowvar=False)
    # ============================================================
    ...
    # associated eigenvalues, sorted in descending order
    # ====================== YOUR CODE HERE ======================
    # Get the indices of the eigenvalues sorted in descending order
    eigenvalue_indices = np.argsort(eigenvalues)[::-1]
    # Sort the eigenvalues and eigenvectors according to the indices
    ...
    # ====================== YOUR CODE HERE ======================
    # My explanation to better understand the code:
    # By analyzing the explained variance ratio, you can determine the number of principal components needed to retain
    # a desired amount of information from the original dataset. It allows you to make an informed decision on how many
    # principal components to select for dimensionality reduction while still preserving a significant portion of the
    # data's variability.
    if k == 0:
        total_variance = np.sum(sorted_eigenvalues)
        explained_variance = np.cumsum(sorted_eigenvalues) / total_variance
        # Adding 1 ensures that we include at least one eigenvector
        k = np.argmax(explained_variance >= 0.95) + 1

    # Select the first k eigenvectors and create the transformation matrix
    ...
    # ====================== YOUR CODE HERE ======================
    iris_data_redux = np.dot(iris_data_normalized, selected_eigenvectors)
# ============================================================
...
if k == 2:
    ...
    for label in names_classes:
        indices_label = iris_df["variety"] == label
        ax_2D.scatter(
            x=iris_data_redux[indices_label, 0],
        ...
        coordinates_label_2D = np.mean(iris_data_redux[indices_label, :], axis=0)
        ax_2D.text(x=coordinates_label_2D[0], y=coordinates_label_2D[1], s=label)
